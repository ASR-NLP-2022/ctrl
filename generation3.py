...
parser.add_argument('--prefix_text', type=str, required=True, help='prefix of the text to gen')
parser.add_argument('--out_file', type=str, required=True, help='File name to save the text')
...
if args.out_file in os.listdir('gen'):
    print(args.out_file, "already exits")
    exit(0)
...
for iters in range(1):
    ...
    try:
        ...
        print(tokens_generated_so_far)
        print()
        with  open('gen/'+args.out_file,'w') as f:
            f.write(tokens_generated_so_far.encode('utf-8'))
        #f.close()
        exit(0)
